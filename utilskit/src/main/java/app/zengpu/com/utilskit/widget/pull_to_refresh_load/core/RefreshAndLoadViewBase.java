...
//                    isScrollToTop = true;
                ...
                if (isBottom() && mYOffset < -mTouchSlop) {
                    isScrollToTop = false;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_MOVE:
                LogUtil.d("RefreshAndLoadViewBase1", "mCurrentStatus ACTION_MOVE  is: " + mCurrentStatus);
                LogUtil.d("RefreshAndLoadViewBase1", "mCurrentStatus isTop  is: " + isTop());
                ...
                changeScrollY(mYOffset * 8 / 10);

                mLastY = currentY;
                break;
            case MotionEvent.ACTION_UP:
                LogUtil.d("RefreshAndLoadViewBase1", "mCurrentStatus ACTION_UP  is: " + mCurrentStatus);
    ...
    protected void changeScrollY(int distance) {
        ...
        if (isScrollToTop && mCurrentStatus != STATUS_LOADING) {
            ...
            } else if (curY > 0 && curY > slop && curY < mInitScrollY) {
                mCurrentStatus = STATUS_PULL_TO_REFRESH;
            } else if (curY > 0 && curY >= mInitScrollY)
                mCurrentStatus = STATUS_IDLE;
            LogUtil.d("RefreshAndLoadViewBase1", "curY refresh  is: " + curY);
        }
    ...
    private OnClickListener loadFailureLisenter = new OnClickListener() {
        ...
        public void onClick(View v) {
            int i = v.getId();
            if (i == R.id.pull_to_refresh_text) {
                refreshing();

            } else if (i == R.id.pull_to_loading_text) {
                loading();
